...
有一个命名需要先说明：我没有在 Z.ai / Hugging Face 官方渠道找到名为 `GLM-5.7` 的开放权重模型卡。现在官方开放权重主线是 `GLM-5`，模型卡里明确说它相比 GLM-4.5 从 355B/32B active 扩展到 744B/40B active，并且 benchmark 中对比的是 GLM-4.7。因此本文按官方名称写 `GLM-5`，不把 `GLM-5.7` 当作独立模型。
...
- 大 MoE：100B 到 1T+，总参数决定权重加载成本，active 参数决定单 token 计算量。代表是 DeepSeek-V4-Pro、Kimi K2.6、GLM-5、Qwen3.5-397B-A17B、Mistral Large 3、Hy3 preview。
...
- Apache 2.0 / MIT：相对宽松，商业使用友好。当前代表包括 Qwen3.5/3.6、GLM-5、Gemma 4、Mistral 3、DeepSeek-V4、gpt-oss。
...
| GLM-5 | MoE + DSA | 744B | 40B | 128K 到 200K+ 评测场景 | MIT | Z.ai 最新开放权重主线，复杂系统工程、长程 agent 任务导向 |
...
GLM 当前官方开放权重主线是 GLM-5，而不是 GLM-5.7。官方模型卡写得很明确：

- GLM-5：744B total / 40B active。
- 相比 GLM-4.5 的 355B total / 32B active，参数规模明显扩大。
- 预训练数据从 23T tokens 增加到 28.5T tokens。
- 集成 DSA，用来降低长上下文部署成本。
- 许可证：MIT。

GLM-5 的定位是复杂系统工程、长程 agentic tasks、代码和推理。它比 GLM-4.5-Air 这类 100B 级模型更重，部署上更接近 DeepSeek-V4-Pro、Kimi K2.6、Qwen3.5-397B 这一档。
...
| 多节点/云推理 | 700B 到 1.6T | GLM-5、Kimi K2.6、DeepSeek-V4-Pro |
...
- 旗舰：Kimi K2.6、GLM-5、DeepSeek-V4-Pro。
...
- 超大型 MoE：Kimi K2.6、GLM-5、DeepSeek-V4-Pro。
...
对于大多数工程场景，Qwen3.6-35B-A3B、Gemma 4 26B-A4B/31B、Ministral 3、gpt-oss-20b/120b 是比较实际的起点。对于追求开放权重最高质量的服务端场景，再考虑 DeepSeek-V4、Kimi K2.6、GLM-5、Qwen3.5-397B-A17B、Mistral Large 3、Hy3 preview 这类大模型。
...
- GLM-5 模型卡：https://huggingface.co/zai-org/GLM-5
